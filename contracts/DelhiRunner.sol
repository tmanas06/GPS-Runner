...
/**
 * @title DelhiRunner
 * @dev GPS Runner contract for Delhi region
 * Stores GPS markers with anti-cheat validation
 * Deploy on Polygon Amoy Testnet
 */
contract DelhiRunner {
    // ============ Constants ============

    // Delhi bounds (multiplied by 1e6 for precision)
    uint256 public constant MIN_LAT = 28400000;  // 28.4°N
    uint256 public constant MAX_LAT = 28800000;  // 28.8°N
    uint256 public constant MIN_LNG = 76900000;  // 76.9°E
    uint256 public constant MAX_LNG = 77400000;  // 77.4°E
    ...
    modifier validCoordinates(uint256 _lat1e6, uint256 _lng1e6) {
        require(
            ...
            "Latitude out of Delhi bounds"
        );
        require(
            _lng1e6 >= MIN_LNG && _lng1e6 <= MAX_LNG,
            "Longitude out of Delhi bounds"
        );
        _;
    }
    ...
    function getLeaderboard(uint256 _limit) external view returns (
        ...
        // Note: This is gas-intensive. For production, use off-chain indexing.
        ...
        // Return empty for now - implement with off-chain indexing
